...
class MyReporter implements Reporter {
  ...
  onBegin(config: FullConfig, suite: Suite) {
    ...
    console.log(
      `Running ${this._testTotal} test${this._testTotal > 1 ? 's' : ''} using ${config.workers} worker${
        config.workers > 1 ? 's' : ''
      }`,
    );
    console.log('\nLegend:');
    console.log('\x1b[32m.\x1b[39m Success');
    console.log("\x1b[33m°\x1b[39m Skipped (it's OK, we don't want to test this)");
    console.log('\x1b[33mx\x1b[39m First failure (maybe a temporary problem, will be retried later)');
    console.log('\x1b[33m±\x1b[39m Success on second run (it was indeed a temporary problem)');
    console.log(
      '\x1b[31mF|T\x1b[39m Second failure (there is a problem to investigate) F is a failed step, T is a timeout meaning the test was to long to finish in time\n',
    );
  }
        ...
        process.stdout.write(
          `\x1b[31m${result.status === 'timedOut' ? 'T' : 'F'}|${
            test.annotations.find((annotation) => annotation.type === 'Url')
              ? test.annotations.find((annotation) => annotation.type === 'Url')?.description
              : test.title
          }|${test.parent.project()?.name}\x1b[39m`,
        );
        this.printEndOfLine();
  ...
  onEnd(result: FullResult) {
    ...
    if (process.env.CI)
      console.log(`See report here: ${process.env.CI_JOB_URL}/artifacts/file/playwright/out/report/index.html`);
